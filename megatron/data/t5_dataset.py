...
class T5Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return build_training_sample(src_tokens,  tgt_tokens,
                                    ...
                                    enc_in_length = enc_in_length, dec_in_length = dec_in_length, current_prop=self.args.current_prop)
...
def build_training_sample(src_tokens,  tgt_tokens,
                        ...
                        multi_task = False, ul2_type = "sample", current_prop=None):
    global RECORDED
    truncated = 0
        ...
        if ul2_type == "batch" or ul2_type == "sample":
            if ul2_type == "sample": 
                if current_prop is not None:
                    ul2_task_id = np_rng.choice([i+1 for i in range(len(ORIGIN_UL2))], size=1, p = current_prop)[0]
                else:
            ...
            # tokenizer = get_tokenizer()
            # print(tokenizer.detokenize(train_sample["text_enc"].tolist()))
            # print(tokenizer.detokenize(train_sample["text_dec"].tolist()))
            # import pdb
            # pdb.set_trace()
            return train_sample
