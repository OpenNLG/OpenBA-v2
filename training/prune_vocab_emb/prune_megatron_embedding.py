...
state_dict_path = "/nvme/qd/ckpt/ori_model/iter_0062000/mp_rank_00/model_optim_rng.pt"

save_dict_path = "/nvme/qd/ckpt/pruned_emb/0/iter_0062000/mp_rank_00/model_optim_rng.pt"
old_spm_path = "/nvme/tokenizer/spiece.model"
new_spm_path = "/nvme/tokenizer/spiece100k.model"
...
def prune_param(state_dict_path,  save_dict_path, ids):
    ...
    embedding = state_dict["model"]["language_model"]["embedding"]['word_embeddings']["weight"]
    new_embedding = torch.index_select(embedding, 0, ids_tensor.to(embedding.device))
    new_dict["model"]["language_model"]["embedding"]['word_embeddings']["weight"] = new_embedding

    bias = state_dict["model"]["lm_head"]["bias"]
    new_bias = torch.index_select(bias, 0, ids_tensor.to(bias.device))
    new_dict["model"]["lm_head"]["bias"] = new_bias
