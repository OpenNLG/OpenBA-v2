...
def get_batch(data_iterator):
    ...
    # import rpdb
    # rpdb.set_trace('0.0.0.0', 1024+torch.distributed.get_rank())
    return tokens_enc, tokens_dec, loss_mask, labels, \
           enc_mask, dec_mask, enc_dec_mask


def loss_func(loss_mask, output_tensors):


    lm_loss_ = output_tensors["lm_loss"].float()
    lm_loss = torch.sum(
        lm_loss_.view(-1) * loss_mask.reshape(-1)) / loss_mask.sum()

    args = get_args()
    loss = lm_loss
    averaged_losses_lm = average_losses_across_data_parallel_group([lm_loss])
    return loss, {'lm loss': averaged_losses_lm[0]}
...
def forward_step(data_iterator, model):
    ...
    output_tensors = model(tokens_enc,
                          tokens_dec,
                          enc_mask,
                          dec_mask,
                          enc_dec_mask,
    ...
    return output_tensors, partial(loss_func, loss_mask)
...
if __name__ == "__main__":
    ...
    # from megatron.model.l0_module import L0Module

    # l0module = L0Module()
    # l0module = l0module.to("cuda:0")

    # import pdb
    # pdb.set_trace()
    # print(l0module.device)
