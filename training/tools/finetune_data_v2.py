...
class Encoder(object):
    ...
    def encode(self, json_line):
        ...
        if Encoder.insturct_column not in data:
            self.cnt += 1
            instruct_text = ""
        else:
            print("WARNING! insturct is in data")
            instruct_text = data[Encoder.insturct_column]
            if instruct_text[0] == '“' and instruct_text[-1] == '”':
                instruct_text = instruct_text[1:-1]
        
        instruct_text = instruct_text.strip()

        source_text = instruct_text + data[Encoder.input_column]
...
def main():
    ...
    for doc, bytes_processed in encoded_docs_batch:
        source, target = doc
        source, target = torch.IntTensor(source), torch.IntTensor(target)
        if len(source) > args.max_length: continue
        builder.add_item(
