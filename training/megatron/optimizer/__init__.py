...
def get_param_groups(modules,
    ...
    for module in modules:
        for name, param in module.named_parameters():
            ...
            if "l0module" in name:
                name_module_l0.append(name)
                if "lambda" in name:
                    lagrange_lr.append(param)
                    name_lag.append(name)
                else:
                    l0module_lr.append(param)
                    name_l0.append(name)
                continue
    ...
    # import pdb
    # pdb.set_trace()
    # l0module parameters load
    if len(l0module_lr):
        param_groups.append({'params': l0module_lr, 'lr_mult': 1000.0})
    if len(lagrange_lr):
        param_groups.append({'params': lagrange_lr, 'lr_mult': -1000.0})
